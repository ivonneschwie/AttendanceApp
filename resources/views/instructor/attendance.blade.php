            ...
            @if (count($attendance) > 0)
                <ul class="divide-y divide-gray-200">
                    @foreach ($attendance as $studentUid => $timestamp)
                        <li class="p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center hover:bg-gray-50 transition">
                            ...
                            <span class="text-sm text-gray-600 mt-2 sm:mt-0">{{ date('F j, Y, g:i a', $timestamp / 1000) }}</span>
                        </li>
                    @endforeach
                </ul>
            @else
